...
<input type="button" value="회원등록" onclick="location.href='${path}/member/signUp'">
...
<c:forEach var="row" items="${list }">
	<tr> <!-- 반복적으로 회원정보를 출력하는 부분 -->
		...
		<td><a href="${path}/admin/memberView?memberId=${row.memberId}">${row.memberName }</a></td> <!-- 이름을 클릭시 상세페이지로 이동, EL은 변수값을 출력 --> 
		<td>${row.admin }</td> 
		<td>${row.regdate }</td>
	</tr>
</c:forEach>
...
<%@ include file="../include/footer.jsp" %> <!-- 해당파일을 연결 -->
